...
class QueryAttributesApplier(queryAttributes: QueryAttributes) {
	...
	def applySchema(identifier: String) : String = 	
		queryAttributes.definition.schemaId.map(schemaId => applyDelimiters(schemaId.schema.schema)+"."+identifier).getOrElse(identifier)
...
object QueryAttributesApplier {
	...
	def none(): QueryAttributesApplier = from(QueryAttributes(None, DBDefinition(None, None), None, Some(1000)))
} 
