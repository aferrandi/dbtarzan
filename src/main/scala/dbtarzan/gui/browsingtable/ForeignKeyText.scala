...
object ForeignKeyText {
  ...
  def buildText(key : ForeignKeyWithSharingCheck) = {
  ...
  def buildTooltip(key: ForeignKey) = {
    s"${key.name}\n- ${buildSide(key.from)}\n- ${buildSide(key.to)}"
  }

  def buildSide(fields: FieldsOnTable) = TableIdLabel.toLabel(fields.table) + fieldsToText(fields.fields)

  private def fieldsIfSharesTable(key : ForeignKeyWithSharingCheck) = Some(key).filter(_.sharesToTable).map(k => fieldsToText(k.key.from.fields))
