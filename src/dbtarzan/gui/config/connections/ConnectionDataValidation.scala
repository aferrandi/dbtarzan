...
object ConnectionDataValidation
{
    ...
    List(
      ...
      Some("Url cannot contain spaces").filter(_ => Validation.containsWhitespace(data.url)), 
      Some("Url must be in URL form").filter(_ => !Validation.isValidJdbcURL(data.url)), 
