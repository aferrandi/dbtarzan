...
class QueryAttributesApplierTest extends AnyFlatSpec {
  ...
  val applier = QueryAttributesApplier.from(QueryAttributes(Some(IdentifierDelimitersValues.squareBrackets), DBDefinition(Some(SchemaId(TestDatabaseIds.databaseId, TestDatabaseIds.simpleDatabaseId, SchemaName("TST"))), None), None))
  ...
  val applier = QueryAttributesApplier.from(QueryAttributes(None, DBDefinition(Some(SchemaId(TestDatabaseIds.databaseId, TestDatabaseIds.simpleDatabaseId, SchemaName("TST"))), None), None))
  assert("TST.TBL" === applier.applySchemaAndDelimiters("TBL"))
  }
