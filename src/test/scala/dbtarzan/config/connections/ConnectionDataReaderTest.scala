...
class ConnectionDataReaderTest extends AnyFlatSpec {
  "A list of connections" should "be parseable" in {
    val values= List(
      ...
      ConnectionData("mysql.jar", "mysql", "DriverMysql", "jdbc://mysql", None, "arturo", Some(Password("fedele")), None, Some(2), Some(IdentifierDelimitersValues.doubleQuotes), Some(300), Some(20), None, Some("catalog"))
      )
