...
class DatabaseInfoExtractorTest extends AnyFlatSpec {
  "databases included completely in all composites of which they are part" should "not appear in databases infos." in {
    ...
    assert(ids.contains(DatabaseId(Left(SimpleDatabaseId("oracle")))))
  }
